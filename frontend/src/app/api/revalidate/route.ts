...
export async function POST(request: NextRequest) {
  ...
  const timestamp = request.headers.get("x-abrit-timestamp") ?? "";
  const supplied = request.headers.get("x-abrit-signature") ?? "";
  ...
  const payload = JSON.parse(rawBody) as { tags?: unknown; paths?: unknown };
  const tags = Array.isArray(payload.tags) ? payload.tags.filter((item): item is string => typeof item === "string").slice(0, 50) : [];
  const paths = Array.isArray(payload.paths) ? payload.paths.filter((item): item is string => typeof item === "string" && item.startsWith("/")).slice(0, 50) : [];
