...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return locales.flatMap((locale) => {
    const paths = ["", "services", "solutions", "products", "pricing", "knowledge", "news", "about", "contact", ...services.map((item) => `services/${item.slug}`), ...solutions.map((item) => `solutions/${item.slug}`)];
