...
export type ManagedItPackage = {
  ...
  whmcs: { productId: number | null; extraUserOptionId: number | null; extraEndpointOptionId: number | null };
};
...
export function buildManagedItWhmcsUrl(product: ManagedItPackage, cycle: ManagedItCycle, extraUsers: number, extraEndpoints: number) {
  if (product.whmcs.productId === null) return null;
  const url = new URL("/cart.php", "https://my.abrit.ir");
  url.searchParams.set("a", "add"); url.searchParams.set("pid", String(product.whmcs.productId)); url.searchParams.set("billingcycle", cycle);
  if (product.whmcs.extraUserOptionId !== null && extraUsers > 0) url.searchParams.set(`configoption[${product.whmcs.extraUserOptionId}]`, String(extraUsers));
  if (product.whmcs.extraEndpointOptionId !== null && extraEndpoints > 0) url.searchParams.set(`configoption[${product.whmcs.extraEndpointOptionId}]`, String(extraEndpoints));
  return url.toString();
}
